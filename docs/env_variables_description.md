...
| `MQTT_PUPPETEER_CORS_ORIGINS` | Dozwolone originy dashboardu (REST i Socket.IO), rozdzielone przecinkami, np. `http://localhost:4200,https://dashboard.example.com`. Wartość `*` odbija dowolny origin (tylko development). Domyślnie `http://localhost:4200`. |
