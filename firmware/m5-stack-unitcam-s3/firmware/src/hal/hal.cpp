...
void hal::cacheWifiMacAddress()
{
    if (!_data.wifi_mac_address.isEmpty())
        return;
    _data.wifi_mac_address = WiFi.macAddress();
    _data.wifi_mac_address.toUpperCase();
    spdlog::info("WiFi MAC cached: {}", _data.wifi_mac_address.c_str());
}
