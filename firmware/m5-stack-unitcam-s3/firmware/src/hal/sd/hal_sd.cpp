...
bool hal::sdCardInit(bool passImagePath)
{
    ...
    if (_sd_spi == nullptr)
    {
        spdlog::error("sd spi allocation failed");
        return false;
    }
    _sd_spi->begin(
        sd_pins.clk,
    ...
    // Init sd 
    bool ret = SD.begin(sd_pins.cs, *_sd_spi, 10000000, "/sd", 12);
    if (!ret)
    {
    ...
    while (1)
    {
        ...
        const int prefixAt = name.lastIndexOf(_img_save_path.prefix);
        const int extensionAt = name.lastIndexOf(_img_save_path.extension);
        if (prefixAt >= 0 && extensionAt > prefixAt)
        {
            const uint32_t existingNumber = name.substring(
                prefixAt + _img_save_path.prefix.length(), extensionAt).toInt();
            image_num = max(image_num, existingNumber + 1);
        }
        file.close();
    }
    dir.close();
...
{
    if (!_data.is_sd_card_valid || img == nullptr || size == 0)
    {
        spdlog::error("cannot save image: invalid SD state or image buffer");
        return false;
    }
    ...
    if (file)
    {
        const size_t written = file.write(img, size);
        file.close();
        if (written != size)
        {
            spdlog::error("incomplete image write: {} of {} bytes", written, size);
            return false;
        }
    }
    else
