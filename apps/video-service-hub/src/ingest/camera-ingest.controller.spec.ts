import type { Request } from 'express';
import { existsSync } from 'node:fs';
import { mkdtemp, rm } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { PassThrough } from 'node:stream';
import type { CameraRegistryService } from '../camera-registry/camera-registry.service';
import { loadServiceConfig } from '../config/service-config';
import { MediaStorageService } from '../storage/media-storage.service';
import { ThumbnailService } from '../thumbnails/thumbnail.service';
import { TranscodingService } from '../transcoding/transcoding.service';
import { CameraIngestController } from './camera-ingest.controller';

// Real, ffmpeg-decodable fixtures -- see thumbnail.service.spec.ts for why a
// hand-rolled SOI/EOI byte pair is not enough to exercise actual generation.
const TINY_JPEG_BASE64 =
  '';
const TINY_WAV_BASE64 =
  '';
const BOUNDARY = 'unitcams3-frame';

function requestFrom(content: Buffer): Request {
  const request = new PassThrough();
  request.end(content);
  return request as unknown as Request;
}

function multipartPart(frameCount: number): Buffer {
  const jpeg = Buffer.from(TINY_JPEG_BASE64, 'base64');
  const chunks: Buffer[] = [];
  for (let index = 0; index < frameCount; index += 1) {
    chunks.push(
      Buffer.from(`--${BOUNDARY}\r\nContent-Type: image/jpeg\r\n\r\n`),
      jpeg,
      Buffer.from('\r\n'),
    );
  }
  return Buffer.concat(chunks);
}

function fakeRegistry(): CameraRegistryService {
  return { tryGet: () => undefined } as unknown as CameraRegistryService;
}

describe('CameraIngestController (eager finalize + thumbnail generation on completion)', () => {
  let storageRoot: string;
  let storage: MediaStorageService;
  let transcoding: TranscodingService;
  let controller: CameraIngestController;

  beforeEach(async () => {
    storageRoot = await mkdtemp(join(tmpdir(), 'video-service-hub-ingest-'));
    const config = loadServiceConfig({
      VIDEO_SERVICE_HUB_STORAGE_PATH: storageRoot,
      VIDEO_SERVICE_HUB_MQTT_PORT: '0',
    });
    storage = new MediaStorageService(config, fakeRegistry());
    await storage.initialize();
    transcoding = new TranscodingService(config, storage);
    controller = new CameraIngestController(
      storage,
      new ThumbnailService(),
      transcoding,
    );
  });

  afterEach(async () => {
    await rm(storageRoot, { recursive: true, force: true });
  });

  it('finalizes and generates an image thumbnail after a single capture is stored', async () => {
    const jpeg = Buffer.from(TINY_JPEG_BASE64, 'base64');
    await controller.capture(
      requestFrom(jpeg),
      'camera-1',
      'capture-1',
      'VGA',
      undefined,
      'image/jpeg',
    );

    expect(
      storage.getMetadata('captures', 'camera-1', 'capture-1'),
    ).toBeDefined();
    const thumbnailPath = storage.thumbnailPath(
      'captures',
      'camera-1',
      'capture-1',
    );
    expect(existsSync(thumbnailPath)).toBe(true);
  }, 30_000);

  it('finalizes a timelapse into an mp4 once every part has arrived', async () => {
    const jpeg = Buffer.from(TINY_JPEG_BASE64, 'base64');
    await controller.timelapsePart(
      requestFrom(jpeg),
      'camera-1',
      'timelapse-1',
      '0',
      '2',
      'VGA',
      undefined,
      'image/jpeg',
    );
    expect(
      storage.getMetadata('timelapses', 'camera-1', 'timelapse-1'),
    ).toBeUndefined();

    await controller.timelapsePart(
      requestFrom(jpeg),
      'camera-1',
      'timelapse-1',
      '1',
      '2',
      'VGA',
      undefined,
      'image/jpeg',
    );

    const metadata = storage.getMetadata(
      'timelapses',
      'camera-1',
      'timelapse-1',
    );
    expect(metadata?.fileName).toBe('timelapse-camera-1.mp4');
    expect(
      existsSync(
        storage.thumbnailPath('timelapses', 'camera-1', 'timelapse-1'),
      ),
    ).toBe(true);
  }, 30_000);

  it('does not finalize a recording until every part has arrived', async () => {
    await controller.recordingPart(
      requestFrom(multipartPart(1)),
      'camera-1',
      'recording-1',
      '0',
      '2',
      'VGA',
      '4',
      '20',
      undefined,
      'multipart/x-mixed-replace; boundary=unitcams3-frame',
    );
    expect(
      storage.getMetadata('recordings', 'camera-1', 'recording-1'),
    ).toBeUndefined();

    await controller.recordingPart(
      requestFrom(multipartPart(1)),
      'camera-1',
      'recording-1',
      '1',
      '2',
      'VGA',
      '4',
      '20',
      undefined,
      'multipart/x-mixed-replace; boundary=unitcams3-frame',
    );

    const metadata = storage.getMetadata(
      'recordings',
      'camera-1',
      'recording-1',
    );
    expect(metadata?.fileName).toBe('recording-camera-1.mp4');
    expect(
      existsSync(
        storage.thumbnailPath('recordings', 'camera-1', 'recording-1'),
      ),
    ).toBe(true);
  }, 30_000);

  it('finalizes a live stream into an mp4 once the stream ends', async () => {
    const upload = new PassThrough();
    const finished = controller.live(
      upload as unknown as Request,
      'camera-1',
      'live-1',
      'VGA',
      `multipart/x-mixed-replace; boundary=${BOUNDARY}`,
    );
    upload.end(multipartPart(2));
    await finished;

    const metadata = storage.getMetadata('live', 'camera-1', 'live-1');
    expect(metadata?.fileName).toBe('live-camera-1.mp4');
    expect(
      existsSync(storage.thumbnailPath('live', 'camera-1', 'live-1')),
    ).toBe(true);
  }, 30_000);

  it('finalizes a single-part audio upload into a wav and generates both waveform variants', async () => {
    const wav = Buffer.from(TINY_WAV_BASE64, 'base64');
    await controller.audioPart(
      requestFrom(wav),
      'camera-1',
      'audio-1',
      '0',
      '1',
      '2',
      undefined,
      'audio/wav',
    );

    const metadata = storage.getMetadata('audio', 'camera-1', 'audio-1');
    expect(metadata?.fileName).toBe('audio-camera-1.wav');
    expect(
      existsSync(storage.audioThumbnailPath('camera-1', 'audio-1', 'dark')),
    ).toBe(true);
    expect(
      existsSync(storage.audioThumbnailPath('camera-1', 'audio-1', 'light')),
    ).toBe(true);
  }, 30_000);
});
