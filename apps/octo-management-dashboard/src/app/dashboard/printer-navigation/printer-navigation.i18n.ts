...
import type { AxisDirection, HotendDirection, PrinterNavigationLabels } from './printer-navigation.models';
...
export function createPrinterNavigationLabels(i18n: I18nService): PrinterNavigationLabels {
  ...
  return {
    ...
    selectPointInstruction: (axis, direction) => i18n.t('navigation.selectPoint', { axis, direction: sign(direction) }),
    activePointHint: (axis, direction) => i18n.t('navigation.point', { axis, direction: sign(direction) }),
    ...
    pointLabel: (axis, direction) => i18n.t('navigation.point', { axis, direction: sign(direction) }).toLocaleLowerCase(i18n.language()),
    ...
    stepOption: (mainStep, alternativeSteps) => `${mainStep}${alternativeSteps.length ? ` (${alternativeSteps.join(', ')})` : ''}`,
    alternativeStepAriaLabel: (axis, direction) => i18n.t('navigation.alternativeStep', { axis, direction: directionName(direction) }),
    hotendStepAriaLabel: (direction) => i18n.t('navigation.hotendStep', { direction: hotendDirection(direction) }),
  };
}
