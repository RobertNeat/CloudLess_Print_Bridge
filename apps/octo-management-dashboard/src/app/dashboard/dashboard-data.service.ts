...
export const MANAGEMENT_DASHBOARD_DATA_SOURCE =
  new InjectionToken<ManagementDashboardDataSource>('MANAGEMENT_DASHBOARD_DATA_SOURCE', {
    providedIn: 'root',
...
function isManagementDashboardData(value: unknown): value is ManagementDashboardData {
  ...
  const { printJob, controls, temperatures, coordinates, navigation, livePreview, widgets, charts } = value;
  return isRecord(printJob)
    && ['name', 'thumbnailUrl', 'thumbnailAlt', 'estimatedPrintTime'].every((key) => isString(printJob[key]))
    && ['progress', 'currentLayer', 'totalLayers'].every((key) => isNumber(printJob[key]))
    && ['printing', 'paused', 'completed', 'cancelled', 'error'].includes(String(printJob['status']))
    && isRecord(controls)
    ...
    && Array.isArray(widgets) && widgets.every(isWidget)
    && isRecord(charts) && ['progress', 'temperature', 'fan'].every((key) => isChart(charts[key]));
}

function isNavigation(value: unknown): boolean {
  if (!isRecord(value) || !isRecord(value['axisPoints']) || !isRecord(value['viewport'])) return false;
  const viewport = value['viewport'];
  return ['X', 'Y', 'Z'].every((axis) => isRecord(value['axisPoints']) && isRecord(value['axisPoints'][axis]))
    && isRecord(value['hotendPoint']) && hasNumbers(value['hotendPoint'], ['x', 'y'])
    && Array.isArray(value['steps']) && value['steps'].every(isNumber)
    && isString(viewport['imageUrl']) && isString(viewport['imageAlt'])
    && isNumber(viewport['width']) && isNumber(viewport['height'])
    && hasNumbers(viewport['viewBox'], ['minX', 'minY', 'width', 'height']);
}

function isLivePreview(value: unknown): boolean {
  return isRecord(value) && isString(value['cameraName']) && isString(value['resolution'])
    && Array.isArray(value['availableResolutions']) && value['availableResolutions'].every(isString)
    && typeof value['active'] === 'boolean' && isNumber(value['latencyMs']);
}
...
function isChart(value: unknown): boolean {
  return isRecord(value)
    ...
    && ['titleKey', 'subtitleKey', 'xAxisLabelKey', 'yAxisLabelKey', 'valueSuffix'].every((key) => isString(value[key]))
    && ['yMin', 'yMax', 'yStepSize'].every((key) => isNumber(value[key]))
    && Array.isArray(value['labels']) && value['labels'].every(isString)
    && Array.isArray(value['datasets']) && value['datasets'].every((dataset) =>
      isRecord(dataset) && isString(dataset['labelKey']) && isString(dataset['colorToken'])
      && Array.isArray(dataset['data']) && dataset['data'].every(isNumber));
}
